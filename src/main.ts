...
export async function run(){
  ...
  //const inputs: context.Inputs = context.getFileInputs()
  //const inputs: context.Inputs = context.getZipInputs()
  //const inputs: context.Inputs = context.getDirInputs()
  //对参数和文件进行校验
  ...
  if(! await fileutils.checkFileContent(inputs.function_codetype,inputs.function_file)){
  ...
  const client = functiongraph.FunctionGraphClient.newBuilder()
    ...
    .withEndpoint(inputs.endpoint).build();
  ...
  if(inputs.function_codetype === "obs"){
    ...
    const base64Content = await fileutils.getArchiveBase64Content(inputs.function_codetype,inputs.function_file);
    funcCodebody.withFile(base64Content);
    body.withFuncCode(funcCodebody);

    let fileName = context.FUNC_TMP_ZIP;
    if(inputs.function_codetype==="jar" || inputs.function_codetype==="zip"){
