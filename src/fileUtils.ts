 ...
 * @returns 
 */
 export async function checkFileContent(fileType:string,filePath:string):Promise<boolean> {
...
export async function getArchiveBase64Content(fileType:string,filePath:string):Promise<string>{
  let archiveFilePath = context.FUNC_TMP_ZIP;
  if(fileType=== "zip" || fileType === "jar"){
    archiveFilePath = filePath;
  }
  let archiveContent:string = await getBase64ZipfileContent(archiveFilePath);
 ...
 */
 ...
 export async function zipDirByPath(dirPath:string){
   let zipDirCommandAll = "mkdir ./tmpdir && cp -r " + dirPath + "/* ./tmpdir "+
                          "&& cd ./tmpdir && zip -r " + context.FUNC_TMP_ZIP + " ./* "+
  ...
  * @returns 
  */
 export async function getBase64ZipfileContent(archiveFilePath: string): Promise<string> {
   let base64Command = "base64 " + archiveFilePath;
   //core.info('command ' + base64Command)
   let base64ZipFileContent = await (cp.execSync(base64Command) || '').toString()
   //core.info('result ' + base64ZipFileContent)
 ...
 export function checkFileSize(filePath:string) : boolean{
  try{
    ...
    if(fileSize > context.MAX_UPLOAD_SIZE){
      core.info("the upload file " + filePath + " size  " + fileSize + " is bigger than 50MB,please upload to OBS first,then deploy by OBS type")
      return false;
    }else{
